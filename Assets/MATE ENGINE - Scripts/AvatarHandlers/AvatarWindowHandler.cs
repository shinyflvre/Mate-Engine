...
public class AvatarWindowHandler : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (!EnsureWindowApiReady() || animator == null || controller == null) return;
        ...
        {
            if (TryGetCursorDesktop(out DesktopPoint cp))
            {
        ...
        if (snappedHWND != IntPtr.Zero)
        {
            ...
            if (!handled && IsWindowUnavailable(snappedHWND)) { ClearSnapAndHide(); }
        }
        if (controller.isDragging)
        {
        ...
        if (snappedHWND != IntPtr.Zero && _postSettleRecalib)
        {
            ...
            {
                if (TryGetDesktopWindowRect(snappedHWND, out DesktopRect tr))
                {
    ...
    void LateUpdate()
    {
        ...
        if (snappedHWND != IntPtr.Zero && controller != null && controller.isDragging)
        {
            FollowSnapped(true);
        }
#endif
    ...
    {
        if (!TryGetCursorDesktop(out DesktopPoint cp)) return true;
    ...
    void UpdateCachedWindows()
    {
        ...
        {
            if (hWnd == unityHWND || !IsWindowVisible(hWnd) || !GetNativeWindowRect(hWnd, out RECT nativeRect)) return true;
            DesktopRect r = ToDesktopRect(nativeRect);
            ...
            if (isTaskbar) { cachedWindows.Add(new WindowEntry { hwnd = hWnd, rect = r, isTaskbar = true, ownerPid = 0 }); return true; }
            ...
            cachedWindows.Add(new WindowEntry { hwnd = hWnd, rect = r, isTaskbar = false, ownerPid = ownerPid });
            return true;
        }, IntPtr.Zero);
#else
        ...
        for (int i = 0; i < windows.Count; i++)
        {
            ...
            if (!window.IsValid || window.OwnerPid == _currentPid) continue;
            cachedWindows.Add(new WindowEntry
            {
                hwnd = window.Id,
                rect = window.Rect,
                isTaskbar = window.IsTaskbarLike,
    ...
    bool IsSitEligibleWindow(IntPtr hWnd, DesktopRect r, System.Text.StringBuilder cls)
    {
#if UNITY_STANDALONE_WIN || UNITY_EDITOR_WIN
    ...
    void TrySnap()
    {
        ...
        for (int i = 0; i < cachedWindows.Count; i++)
        {
            ...
            if (TryGetCursorDesktop(out DesktopPoint cp)) _snapCursorY = cp.Y;
            ...
            if (TryGetDesktopWindowRect(win.hwnd, out DesktopRect tr)) PinToTarget(tr); else PinToTarget(win.rect);
            return;
        }
    }
    ...
    {
        if (snappedHWND == IntPtr.Zero || !TryGetDesktopWindowRect(snappedHWND, out DesktopRect tr)) { ClearSnapAndHide(); return; }
    ...
    void PinToTarget(DesktopRect r)
    {
        ...
        if (!TryGetOwnWindowRect(out DesktopRect ur)) return;
        ...
        {
            if (dx != 0 || dy != 0) MoveOwnWindow(targetX, targetY, w, h, true);
            return;
        }
        ...
        if (nx != ur.Left || ny != ur.Top) MoveOwnWindow(nx, ny, w, h, true);
    }
    ...
    bool IsOccludedByHigherWindowsAtPoint(IntPtr hwnd, int x, int y)
    {
        ...
        while (h != IntPtr.Zero)
        {
            ...
            if (!IsWindowVisible(h) || IsCloaked(h) || !GetNativeWindowRect(h, out RECT nativeRect)) { h = GetWindow(h, GW_HWNDPREV); continue; }
            DesktopRect r = ToDesktopRect(nativeRect);
        ...
        return false;
#else
        return windowApi != null && windowApi.IsPointOccludedByHigherWindow(hwnd, x, y, null);
#endif
    }
    ...
    void UpdateOccluderQuadsFrameSync()
    {
        ...
        if (snappedHWND != unityHWND && TryGetDesktopWindowRect(snappedHWND, out DesktopRect tr))
        {
        ...
        for (int i = 0; i < activeOccluders.Count && outCount < maxOtherQuads; i++)
        {
            ...
            if (!TryGetDesktopWindowRect(w.hwnd, out DesktopRect wrct)) continue;
    ...
    bool IsAboveInZOrder(IntPtr a, IntPtr b)
    {
        ...
        return false;
#else
        return windowApi != null && windowApi.IsAbove(a, b);
#endif
    }
    ...
    {
        return TryGetOwnWindowRect(out DesktopRect r) ? new Vector2(r.Left, r.Top) : Vector2.zero;
    }
    ...
    bool GetUnityClientRect(out DesktopRect r)
    {
        ...
        return windowApi != null && windowApi.TryGetOwnClientRect(out r);
    }
    ...
    bool IsWindowMaximized(IntPtr hwnd)
    {
        ...
        return false;
#else
        return windowApi != null && windowApi.IsWindowMaximized(hwnd);
#endif
    }
...
#else
        return windowApi != null && windowApi.IsWindowFullscreen(new DesktopWindowInfo { Id = win.hwnd, Rect = win.rect, IsTaskbarLike = win.isTaskbar });
#endif
    }

    bool EnsureWindowApiReady()
    {
        if (windowApi == null) windowApi = DesktopWindowApi.Current;
        if (windowApi == null || !windowApi.IsSupported) return false;
        bool ready = windowApi.RefreshOwnWindow();
        if (ready && unityHWND == IntPtr.Zero) unityHWND = GetOwnWindowId();
        return ready;
    }

    IntPtr GetOwnWindowId()
    {
#if UNITY_STANDALONE_WIN || UNITY_EDITOR_WIN
    ...
    bool TryGetCursorDesktop(out DesktopPoint point)
    {
        point = new DesktopPoint();
        if (windowApi == null) windowApi = DesktopWindowApi.Current;
        return windowApi != null && windowApi.TryGetCursorPosition(out point);
    }
    ...
    bool TryGetOwnWindowRect(out DesktopRect rect)
    {
        ...
        if (windowApi == null) windowApi = DesktopWindowApi.Current;
        return windowApi != null && windowApi.TryGetOwnWindowRect(out rect);
    }

    bool TryGetDesktopWindowRect(IntPtr windowId, out DesktopRect rect)
    {
        rect = new DesktopRect();
        if (windowId == IntPtr.Zero) return false;
#if UNITY_STANDALONE_WIN || UNITY_EDITOR_WIN
        if (GetNativeWindowRect(windowId, out RECT nativeRect))
        {
            rect = ToDesktopRect(nativeRect);
            return !rect.IsEmpty;
        }
        return false;
#else
        if (windowApi == null) windowApi = DesktopWindowApi.Current;
        return windowApi != null && windowApi.TryGetWindowRect(windowId, out rect);
#endif
    }

    bool MoveOwnWindow(int x, int y, int width, int height, bool repaint)
    {
        if (windowApi == null) windowApi = DesktopWindowApi.Current;
        return windowApi != null && windowApi.TryMoveOwnWindow(x, y, width, height, repaint);
    }

    bool IsWindowUnavailable(IntPtr windowId)
    {
...
#if UNITY_STANDALONE_WIN || UNITY_EDITOR_WIN
    static DesktopRect ToDesktopRect(RECT r) => new DesktopRect(r.Left, r.Top, r.Right, r.Bottom);
#endif
    ...
    [DllImport("user32.dll", EntryPoint = "GetWindowRect")] static extern bool GetNativeWindowRect(IntPtr hWnd, out RECT lpRect);
